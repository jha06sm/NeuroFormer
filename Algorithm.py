...
from DataAdapter.DataAdapter import TrainAdapter,TestAdapter,ValAdapter,BaseAdapter
...
class Algorithm():
    ...
    def __init__(self,model_save_path,log_save_path,split_data_file_path,batch_size,learning_rate,epochs,cuda_device,reg_para,parallel = True):
        ...
        # init model
        self.model = EpilepsyWithDilatedConv()
        if self.parallel:
    ...
    def train(self):
        ...
        val_adapter = ValAdapter(self.base_adapter)
        val_loader = Data.DataLoader(val_adapter,batch_size=self.batch_size,shuffle=False,num_workers=0)
        ...
        for epoch in range(1,self.epochs + 1):
            ...
            self.cal_batch(val_loader,val_metric,'valid')
            end_time = time.time()
            self.log.log('- Epoch: {:d} - Train_loss: {:.5f} - Train_mean_acc: {:.5f} - Val_loss: {:.5f} - Val_mean_acc: {:.5f} - T_Time: {:.3f}'.format\
                (epoch,train_metric.get_loss(),train_metric.get_accuracy(),val_metric.get_loss(),val_metric.get_accuracy(),end_time - start_time))
